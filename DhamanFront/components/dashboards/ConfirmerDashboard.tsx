...
const ConfirmerDashboard = () => {
      ...
      prev.map((order) => {
        if (order._id === id) {
          ...
          if (newAttempts >= 6) {
            Alert.alert(
              "نظام ضمان",
              "تم إلغاء الطلب تلقائياً (6 محاولات بدون رد)",
            );
  ...
  const handlePostpone = (id: string) => {
    Alert.prompt("تأجيل الطلبية", "أدخل تاريخ التأجيل (مثال: 2024-06-01)", [
        ...
        onPress: (date) => {
          if (date) {
            setOrders((prev) =>
              prev.map((o) =>
                o._id === id
                  ? { ...o, status: "POSTPONED", postponedDate: date }
                  : o,
              ),
            );
          }
      ...
      setOrders(
        orders.map((o) =>
          o._id === editingOrder._id
            ? { ...o, ...formData, totalAmount: Number(formData.totalAmount) }
            : o,
        ),
      );
    } else {
        ...
        {orders
          ...
          .map((order: any) => (
            ...
            >
              {/* Status Badge */}
              <View
                className={`absolute top-0 left-0 px-4 py-1 rounded-br-2xl ${
              ...
              <View className="flex-row justify-between items-start mb-4 pt-2">
                ...
                <View className="items-end flex-1 ml-4">
                  <Text className="text-[9px] font-black text-slate-300">
                    #{order._id.slice(-6)} • {order.createdAt}
                  </Text>
                  <Text className="text-md font-black text-slate-800 mt-0.5">
                    {order.customerName}
                  </Text>
                  <TouchableOpacity
                    onPress={() =>
                  ...
                  >
                    <Text className="text-[11px] font-bold text-slate-500 mr-2">
                      {order.customerPhone}
                    </Text>
              ...
              <View className="bg-slate-50 rounded-2xl p-3 mb-4 space-y-2">
                ...
                {order.postponedDate && (
                  <View className="flex-row justify-between bg-blue-50 p-1 rounded-lg">
                    ...
                    <Text className="text-[10px] font-black text-blue-400">
                      : تاريخ التأجيل
                    </Text>
                  </View>
                )}
              ...
              <View className="flex-row flex-wrap gap-2">
                ...
                >
                  <Text className="text-white font-black text-[10px] mr-2">
                    تأكيد
                  </Text>
              ...
              <View className="mt-4 pt-3 border-t border-slate-50 flex-row justify-between items-center">
                ...
                >
                  <Text className="text-[10px] font-bold text-slate-400 mr-1">
                    تعديل البيانات
                  </Text>
          ...
          <View className="bg-white rounded-t-[3rem] p-8 h-[80%]">
            <View className="flex-row justify-between items-center mb-6">
              ...
              <Text className="text-xl font-black text-slate-800">
                {editingOrder ? "تعديل طلبية" : "إضافة طلبية"}
              </Text>
            </View>
            <ScrollView className="space-y-4">
              ...
              <TextInput
                ...
                onChangeText={(t) =>
                  setFormData({ ...formData, productName: t })
                }
              />
              <TextInput
                className="w-full p-4 bg-slate-50 rounded-2xl text-right font-bold border border-slate-100 mb-3"
                ...
                onChangeText={(t) =>
                  setFormData({ ...formData, totalAmount: t })
                }
              />
              <TouchableOpacity
                onPress={handleSaveOrder}
                className="bg-slate-900 py-4 rounded-2xl flex-row justify-center items-center"
              >
