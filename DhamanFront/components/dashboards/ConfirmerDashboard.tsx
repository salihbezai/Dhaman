...
const ConfirmerDashboard = () => {
  ...
  useEffect(() => {
    ...
    socket.on("NEW_NOTIFICATION_DRIVER", async (data) => {
      ...
      Alert.alert("تنبيه من السائق", "وصل السائق إلى وجهة الطلبية");
    });
  ...
  const validateForm = () => {
    if (!formData.customerName.trim()) { Alert.alert("خطأ", "يرجى إدخال إسم الزبون"); return false; }
    if (!formData.customerPhone.trim() || formData.customerPhone.length < 10) { Alert.alert("خطأ", "يرجى إدخال رقم هاتف صحيح"); return false; }
    if (!formData.address.trim()) { Alert.alert("خطأ", "العنوان مطلوب لتوصيل الطلبية"); return false; }
    if (!formData.wilaya.trim()) { Alert.alert("خطأ", "الولاية مطلوبة لتوصيل الطلبية"); return false; }
    if (formData.items.length === 0) { Alert.alert("خطأ", "لا يمكن حفظ طلبية فارغة من المنتجات"); return false; }
    return true;
  };

  const handleConfirmOrder = (id: string) => {
    Alert.alert("تأكيد", "تأكيد الطلب؟", [{ text: "تراجع" }, { text: "تأكيد", onPress: async () => await dispatch(handleConfirmTheOrder({ id })) }]);
  };

  const handleCancel = (id: string) => {
    Alert.alert("إلغاء", "تأكيد إلغاء الطلب؟", [{ text: "تراجع" }, { text: "إلغاء", onPress: async () => await dispatch(handleCancelOrder({ id })) }]);
  };

  const handleNoAnswerAction = async (order: any) => {
    const currentAttempts = order.callAttempts || 0;
    const nextAttempts = currentAttempts + 1;
    Alert.alert("تنبيه", "هل تريد تسجيل حالة عدم رد؟", [{ text: "تراجع" }, { text: "حفظ", onPress: async () => await dispatch(handleNoAnswerOrder({ id: order._id, newAttempts: nextAttempts })) }]);
  };

  const handlePostpone = (id: string) => { setSelectedOrderId(id); setShowDatePicker(true); };

  const onDateChange = (event: any, selectedDate?: Date) => {
    setShowDatePicker(false);
    if (selectedDate && selectedOrderId) {
      dispatch(handlePostponeOrder({ id: selectedOrderId, postponedDate: selectedDate.toISOString() }));
  ...
  const handleSaveOrder = async () => {
    ...
    if (result.meta.requestStatus === "fulfilled") { setShowForm(false); Alert.alert("نجاح", "تمت العملية بنجاح"); }
  };

  const handleRemoveOrder = async (order: any) => {
    Alert.alert("تنبيه", "هل تريد حقا حذف هذه الطلبية؟", [{ text: "تراجع" }, { text: "حذف", onPress: async () => await dispatch(handleRemoveOrderByConfirmer({ id: order._id })) }]);
  };
    ...
    <View className="flex-1 bg-slate-50" style={{ direction: "rtl" }}>
      ...
      <View className="bg-slate-900 pt-14 pb-6 px-6 rounded-b-[3rem] shadow-2xl">
        <View className="flex-row-reverse justify-between items-center mb-8">
          <View className="flex-row-reverse items-center gap-3">
            <TouchableOpacity
              onPress={() => {
                ...
                setFormData({ _id: "", customerName: "", customerPhone: "", totalAmount: 0, wilaya: "باتنة", address: "حي الرياض", deliveryPrice: parseInt(defaultWilaya?.deliveryPrice || "0"), items: [] });
                setShowForm(true);
              }}
          ...
          <View className="flex-row-reverse items-center">
            <View className="mr-3 items-start">
              ...
              <Text className="text-emerald-500 text-[14px] font-bold">مرحباً، {confirmer?.username} 👋</Text>
            </View>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} className="flex-row">
          {["all", "PENDING", "CONFIRMED", "DELIVERED", "POSTPONED", "CANCELLED"].map((tab) => (
            <TouchableOpacity key={tab} onPress={() => setActiveTab(tab)} className={`px-5 py-2.5 rounded-2xl ml-2 ${activeTab === tab ? "bg-emerald-500" : "bg-white/10"}`}>
              <Text className={`text-[11px] font-black ${activeTab === tab ? "text-white" : "text-white/60"}`}>
                {tab === "all" ? "الكل" : tab === "PENDING" ? "معلقة" : tab === "CONFIRMED" ? "مؤكدة" : tab === "DELIVERED" ? "تم التسليم" : tab === "POSTPONED" ? "مؤجلة" : "ملغاة"}
              </Text>
            </TouchableOpacity>
          ))}
        </ScrollView>
      ...
      >
        {loading && !refreshing ? (
          ...
          <View>
            {orders.filter((o: any) => activeTab === "all" || o.status === activeTab).map((order: any) => (
              <View key={order._id} className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm p-5 mb-4 relative overflow-hidden">
                ...
                <View className="flex-row justify-between items-end mb-6">
                  <View className="bg-slate-50 p-3 rounded-2xl border border-slate-100 items-center min-w-[100px]">
                    ...
                    <Text className="text-[10px] font-bold text-slate-400 uppercase">المجموع</Text>
                  </View>
                ...
                <View className="mt-5 pt-4 border-t border-slate-50 flex-row justify-between items-center">
                  <TouchableOpacity onPress={() => handleRemoveOrder(order)}><Trash2 size={18} color="#fca5a5" /></TouchableOpacity>
                  <TouchableOpacity onPress={() => { setEditingOrder(order); setFormData({ _id: order._id, customerName: order.customerName, customerPhone: order.customerPhone, totalAmount: order.totalAmount, wilaya: order.wilaya, address: order.address, deliveryPrice: order.deliveryPrice, items: [...order.items] }); setShowForm(true); }} className="flex-row items-center bg-slate-50 px-3 py-1.5 rounded-lg">
        ...
        <View className="flex-1 justify-end bg-black/40">
          <View className="bg-white rounded-t-[3rem] p-8 shadow-2xl">
            ...
            <View className="flex-row justify-between items-center mb-6">
              <Text className="text-slate-900 text-xl font-black">تفاصيل الطلبية</Text>
              <View className="bg-slate-100 px-3 py-1 rounded-full">
                <Text className="text-slate-500 font-bold text-[12px]">#{viewingOrder?.orderNumber}</Text>
              </View>
            </View>
            ...
            <ScrollView showsVerticalScrollIndicator={false} className="mb-4">
              ...
              <View className="bg-slate-50 rounded-3xl p-5 mb-4 border border-slate-100">
                <View className="flex-row-reverse items-center mb-4">
                  <View className="mr-3 items-start flex-1">
                    <Text className="text-slate-400 text-[10px] font-bold uppercase">الزبون</Text>
                    <Text className="text-slate-800 font-black text-[16px]">{viewingOrder?.customerName}</Text>
                  </View>
                </View>

                <TouchableOpacity 
                  onPress={() => Linking.openURL(`tel:${viewingOrder?.customerPhone}`)}
                ...
                >
                  ...
                  <View className="mr-3 items-start flex-1">
                    <Text className="text-slate-400 text-[10px] font-bold uppercase">رقم الهاتف</Text>
                    <Text className="text-blue-600 font-black text-[16px]">{viewingOrder?.customerPhone}</Text>
                  </View>
                </TouchableOpacity>
                ...
                <View className="flex-row-reverse items-center">
                  ...
                  <View className="mr-3 items-start flex-1">
                    <Text className="text-slate-400 text-[10px] font-bold uppercase">العنوان والولاية</Text>
                    <Text className="text-slate-800 font-black text-[14px]">{viewingOrder?.wilaya} - {viewingOrder?.address}</Text>
                  </View>
                </View>
              </View>
              ...
              <View className="bg-slate-50 rounded-3xl p-5 border border-slate-100">
                ...
                {viewingOrder?.items?.map((item: any, idx: number) => (
                  <View key={idx} className={`flex-row-reverse justify-between items-center py-3 ${idx !== viewingOrder.items.length - 1 ? "border-b border-slate-200/50" : ""}`}>
                    <View className="bg-emerald-100 px-3 py-1 rounded-lg">
                      <Text className="text-emerald-700 font-black text-[14px]">
                        {item.priceAtTimeOfOrder} دج <Text className="text-red-600">x{item.quantity}</Text>
                      </Text>
                    </View>
                    <Text className="text-slate-700 font-bold text-[16px] flex-1 ml-2 text-right">{item.productName}</Text>
                  </View>
                ))}

                <View className="mt-4 pt-4 border-t border-slate-200 flex-row justify-between items-center">
                   <Text className="text-slate-400 font-bold">المجموع الإجمالي مع التوصيل</Text>
                </View>
                   <Text className="text-red-600 font-black text-lg text-center">{viewingOrder?.totalAmount} دج</Text>
              </View>
            </ScrollView>
      ...
      <Modal visible={showForm} animationType="fade" transparent>
        <View className="flex-1 bg-black/60 justify-center p-4">
          <View className="bg-white rounded-[2.5rem] p-6 shadow-xl max-h-[90%]">
            <Text className="text-slate-900 text-xl font-black mb-4">{formData._id ? "تعديل طلبية" : "إضافة طلبية"}</Text>
            <ScrollView showsVerticalScrollIndicator={false}>
              <Text className="text-slate-400 font-bold mb-1">إسم الزبون</Text>
              <TextInput className="bg-slate-50 border border-slate-100 rounded-xl p-3 mb-3 font-bold" value={formData.customerName} onChangeText={(t) => setFormData({ ...formData, customerName: t })} />
              <Text className="text-slate-400 font-bold mb-1">رقم الهاتف</Text>
              <TextInput className="bg-slate-50 border border-slate-100 rounded-xl p-3 mb-3 font-bold" keyboardType="phone-pad" value={formData.customerPhone} onChangeText={(t) => setFormData({ ...formData, customerPhone: t })} />
              <Text className="text-slate-400 font-bold mb-1 text-right">الولاية</Text>
              <TouchableOpacity onPress={() => setShowWilayaPicker(true)} className="bg-slate-50 border border-slate-100 rounded-xl p-4 mb-3 flex-row justify-between items-center"><MapPin size={18} color="#64748b" /><Text className="font-bold text-slate-700">{formData.wilaya}</Text></TouchableOpacity>
              <Text className="text-slate-400 font-bold mb-1">العنوان</Text>
              <TextInput className="bg-slate-50 border border-slate-100 rounded-xl p-3 mb-3 font-bold" multiline value={formData.address} onChangeText={(t) => setFormData({ ...formData, address: t })} />
              <View className="bg-red-50 p-4 rounded-xl items-center mb-4"><Text className="text-red-600 font-black text-lg">{formData.totalAmount} دج</Text></View>
              <TouchableOpacity onPress={() => setShowProductPicker(true)} className="flex-row items-center justify-center bg-emerald-50 border border-emerald-200 border-dashed py-3 rounded-xl mb-4"><Plus size={18} color="#10b981" /><Text className="text-emerald-700 font-black mr-2">إضافة منتج</Text></TouchableOpacity>
            </ScrollView>
            <View className="flex-row gap-2 mt-4">
              <TouchableOpacity onPress={() => setShowForm(false)} className="flex-1 bg-slate-100 py-4 rounded-2xl items-center"><Text className="text-slate-500 font-black">إلغاء</Text></TouchableOpacity>
              <TouchableOpacity onPress={handleSaveOrder} className="flex-1 bg-emerald-500 py-4 rounded-2xl items-center"><Text className="text-white font-black">حفظ</Text></TouchableOpacity>
            </View>
          </View>
        </View>
      </Modal>
      ...
      <Modal visible={showNotifications} animationType="slide" transparent onRequestClose={() => setShowNotifications(false)}>
        <View className="flex-1 justify-end bg-black/40">
          <View className="bg-white rounded-t-[3rem] h-[75%] p-8 shadow-2xl">
            <View className="w-12 h-1.5 bg-slate-200 rounded-full self-center mb-6" />
            <View className="flex-row justify-between items-center mb-6">
              <Text className="text-slate-900 text-xl font-black">التنبيهات</Text>
              <TouchableOpacity onPress={() => setShowNotifications(false)}><XCircle size={24} color="#64748b" /></TouchableOpacity>
            </View>
            ...
            <ScrollView showsVerticalScrollIndicator={false}>
              {notifications.length === 0 ? (
                <View className="items-center py-20">
                  ...
                  <Text className="text-slate-400 font-bold mt-4">لا توجد تنبيهات حالياً</Text>
                </View>
              ) : (
                  ...
                  <TouchableOpacity
                    ...
                    onPress={async () => {
                      if (notif.isRead === false) {
                        ...
                        Alert.alert("تنبيه", "  التنبيه" );
                      }
                      const linkedOrder = orders.find(o => o._id === notif.orderId);
                      if (linkedOrder) { setViewingOrder(linkedOrder); setShowNotifications(false); }
                    }}
                  >
                    <View className="flex-row-reverse justify-between items-center mb-2">
                      <View className="flex-row-reverse items-center">
                        <MapPin size={16} color="#10b981" />
                        <Text className="text-slate-900 font-black mr-2">وصل السائق</Text>
                      </View>
                      <Text className="text-[10px] text-slate-400 font-bold">{new Date(notif.createdAt).toLocaleTimeString('ar-DZ')}</Text>
                    </View>
                    <Text className="text-slate-600 text-[12px] text-right">لقد وصل السائق إلى موقع الزبون للطلبية رقم #{notif.orderId?.slice(-6)}</Text>
                  </TouchableOpacity>
                ))
              )}
            </ScrollView>
            <TouchableOpacity onPress={() => setShowNotifications(false)} className="bg-slate-900 py-4 rounded-2xl items-center mt-4"><Text className="text-white font-black">إغلاق</Text></TouchableOpacity>
          </View>
        </View>
      </Modal>
